...
description: "An analysis of historical tornado patterns in the area of Twin Ciries from 2000-Current(2025) to predict Tornado Occurence"
format:
   html:
    code-tools: true
    code-overflow: wrap
...
The goal of this project is to use publically available daily weather data to predict tornado formation in the Twin Cities area. Using 2 datasets, the NOAA historical Tornado Database for Minnesota and historical Weather data out of MSP International Airport from 2000 - 2025 the aim is to train and test models that can classify tornado occurance based off of daily weather data. This led me to ask two questions seen below: 
...
To see if this is possible four models are going to be trained; Logistic Regression, for its basic approach and easy interpretabilty; Naive Bayes, for its small hyperparameter space; Random Forest, for its more complex analysis; and lastly Vector Machines, for its high dimensional efectiveness. Once the models are trained a best model will be selected based off a trade off between correctly classifying most to all of the tornados and false tornado alarms. Once the best model is picked a simple GUI with sliders will allow users to try their hand at differentiating parameters. 

## Intro

The primary goal of this project is to use publically available daily weather data to form relationships with NOAA records of tornado formation. This is crucial because unlike hurricanes which have a longer warning timeline, tornadoes are often learned about shortly before they occur. This can cause more extreme damage as people do not have time to secure crucial belongins or themselves in a non-affected area. Daily weather forecast are available and usually a part of every person's daily life. Whether that be through a weather app or the news this publically available data could be potentially utilized to help warn/inform people of the chance of tornado occurring in their areas. One approach to this problem is to create classification models such as Logistic Regression, Random Forest, Naive Bayes, etc that can classify tornado conditions based off historical daily weather data. Once models exists the hope would be to be able to forecast dangerous tornado data based of the daily weather forecast on say a weather app. 
...
Below is a simple application/GUI meant to take the best model and predict if a tornado has a high chance to spawn. Using this app one could hopefully plug in forecasted data and attempt to understand if it is likely a tornado will occurr in their area. DUE TO TIME CONSTRAINTS I WAS UNABLE TO UN NORMALIZE THE PARAMETERS FOR MAX AND MIN RANGE THIS IS A SHORTCOMING BUT I DID NOT WANT IT TO LIMIT FROM THE WORK I DID DO. 
```{python}
#| label: app
#| message: false
